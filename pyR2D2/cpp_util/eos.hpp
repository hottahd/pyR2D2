...
struct EOS
{
  py::array_t<float> log_ro_np;
  py::array_t<float> se_np;

  std::array<py::array_t<float>, 4> log_eos_np;
  ...
  EOS(
      py::array_t<float> log_ro_np_,
      py::array_t<float> se_np_,
      py::array_t<float> log_pr_np_,
      py::array_t<float> log_en_np_,
      py::array_t<float> log_te_np_,
      py::array_t<float> log_op_np_)
  ...
  float inline _eval_core(float ro_val, float se_val, const ViewArray<float> &log_ro, const ViewArray<float> &se, const ViewArray<float> &log_eos) const
  {
    float log_ro_val = std::log(ro_val);
    ...
    float dlog_ro_val = (log_ro_val - log_ro(i));
    float dse_val = (se_val - se(j));
  ...
  float eval(float ro_val, float se_val, const std::string &var_name) const
  {
    auto log_ro = view_array<float>(log_ro_np);
    auto se = view_array<float>(se_np);
    auto log_eos = view_array<float>(log_eos_np[var_id(var_name)]);

    return _eval_core(ro_val, se_val, log_ro, se, log_eos);
  };

  py::array_t<float> eval(
      const py::array_t<float, py::array::c_style | py::array::forcecast> &ro_val_np,
      const py::array_t<float, py::array::c_style | py::array::forcecast> &se_val_np,
      const std::string &var_name) const
  {
    ...
    if (ro_val.ndim == 1)
      qq_np = py::array_t<float>(ro_val.i_size);
    else if (ro_val.ndim == 2)
      qq_np = py::array_t<float>({ro_val.i_size, ro_val.j_size});
    else if (ro_val.ndim == 3)
      qq_np = py::array_t<float>({ro_val.i_size, ro_val.j_size, ro_val.k_size});
    else
      throw std::runtime_error("only 1D, 2D, and 3D arrays are supported");
    auto qq = view_array<float>(qq_np);
