...
def gen_coord_ununiform_flex(xmax, xmin, ix, margin, dx_fine, ix_fine, xc_fine):
    ...
    for i in range(1, ix):
        ...
        if (
            i == ix_fine / 2 or i == ix_fine / 2 + ix_low
        ):  # for fine and coarse grid boundary
...
def regrid_data(
    ...
    if data.datadir.parts[-2] == caseid:
    ...
    if os.path.exists("../run/" + caseid):
        ...
        os.system('rsync -avP --exclude="data" ' + data.datadir.parent / "run" / caseid)
    ...
    for m in tqdm(range(0, data.mtype)):
        if memory_saving:
            up.qq = pyR2D2.fortran_util.interp(
                data.xg, data.yg, data.zg, up.x, up.y, up.z, data.qc[:, :, :, m]
            )
            up.qq.reshape([up.ixg * up.jxg * up.kxg], order="F").astype(
                endian + "d"
            ).tofile(
                "../run/" + caseid + "/data/qq/qq" + "{0:02d}".format(m) + ".dac.e"
            )
        else:
            up.qq[:, :, :, m] = pyR2D2.fortran_util.interp(
                data.xg, data.yg, data.zg, up.x, up.y, up.z, data.qc[:, :, :, m]
            )

    if not memory_saving:
        print("### Save data ###")
        up.qq.reshape(data.mtype * up.ixg * up.jxg * up.kxg, order="F").astype(
            endian + "d"
    ...
    t = np.zeros(1)
    t.reshape([1], order="F").astype(endian + "d").tofile(
        "../run/" + caseid + "/data/time/mhd/t.dac.e"
    )
    ...
    f.write(
        "The initial condition of this run is upgraded data from the other directory\n"
    )
    f.write("This file describes the original and the upgraded data\n\n")
    f.write("### Original data ###\n\n")
    f.write("Server: " + data.server + "\n")
    f.write("Datadir: " + str(data.datadir) + "\n")
    if end_step:
        cstep = str(data.nd)
    else:
        cstep = str(n)
    ...
    f.write(
        ...
        + "{:.4e}".format(abs(value))
        + " or "
        + "{:.3f}".format(data.xmin / data.rstar)
        + "*rstar"
    ...
    f.write(
        ...
        + "{:.4e}".format(abs(value))
        + " or "
        + "{:.3f}".format(data.xmax / data.rstar)
        + "*rstar"
    ...
    if data.geometry == "Spherical":
        ...
        f.write(
            "ymin = "
            + "{:.4e}".format(data.ymin / np.pi * 180)
            + " [rad] "
            + change_judge(ymin, data, "ymin", enp=False)
            + "\n"
        )
        f.write(
            "ymax = "
            + "{:.4e}".format(data.ymax / np.pi * 180)
            + " [rad] "
            + change_judge(ymax, data, "ymax", enp=False)
            + "\n"
        )
        f.write(
            "zmin = "
            + "{:.4e}".format(data.zmin / np.pi * 180)
            + " [rad] "
            + change_judge(zmin, data, "zmin", enp=False)
            + "\n"
        )
        f.write(
            "zmax = "
            + "{:.4e}".format(data.zmax / np.pi * 180)
            + " [rad] "
            + change_judge(zmax, data, "zmax", enp=False)
            + "\n"
        )
    else:
        f.write(
    ...
    f.write(
        "nx0*ix0 = " + str(data.ix) + change_judge(up.ix, data, "ix", enp=False) + "\n"
    )
    f.write(
        "ny0*jx0 = " + str(data.jx) + change_judge(up.jx, data, "jx", enp=False) + "\n"
    )
    f.write(
        "nz0*kx0 = " + str(data.kx) + change_judge(up.kx, data, "kx", enp=False) + "\n"
    )
    ...
    f.write(
        "ix_ununi = "
        + str(data.ix_ununi)
        + change_judge(ix_ununi, data, "ix_ununi", enp=False)
        + "\n"
    )
    f.write(
        "dx00 = "
        "{:.4e}".format(data.dx00) + change_judge(dx00, data, "dx00", enp=False) + "\n"
    )
    ...
    f.write(
        "nx0*ix0 = " + str(up.ix) + change_judge(up.ix, data, "ix", enp=False) + "\n"
    )
    f.write(
        "ny0*jx0 = " + str(up.jx) + change_judge(up.jx, data, "jx", enp=False) + "\n"
    )
    f.write(
        "nz0*kx0 = " + str(up.kx) + change_judge(up.kx, data, "kx", enp=False) + "\n"
    )
    ...
    f.write(
        "dx00 = "
        "{:.4e}".format(dx00) + change_judge(dx00, data, "dx00", enp=False) + "\n"
    )
    ...
    if data.geometry == "Spherical":
        print(
            "ymin = " + "{:.4e}".format(ymin / np.pi * 180),
            change_judge(ymin, data, "ymin"),
        )
        print(
            "ymax = " + "{:.4e}".format(ymax / np.pi * 180),
            change_judge(ymax, data, "ymax"),
        )
        print(
            "zmin = " + "{:.4e}".format(zmin / np.pi * 180),
            change_judge(zmin, data, "zmin"),
        )
        print(
            "zmax = " + "{:.4e}".format(zmax / np.pi * 180),
            change_judge(zmax, data, "zmax"),
        )
    else:
    ...
    print(
        "uniform_flag = " + uniform_flag, change_judge(x_ununif, data, "ununiform_flag")
    )
